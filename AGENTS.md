...
investigate + plan → architect (stop for review if blast ≥ 1) · implement → builder · find/grep/summarize → scout, spawned in parallel · verify → verifier (never the agent that wrote the code) · final review of a blast-2 change → architect.
...
- **Shift DOWN:** plan approved → builder. A part becomes a lookup, rename, grep, or summary → scout. A retry passed → the next part resumes its natural tier; escalation never sticks to the whole task. Verification is mechanical → verifier at default.
